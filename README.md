...
cd Dual_UE_Operation_by_using_OAI/repo/configs
docker compose up -d
...
With the core containers up, start the gNB process (on the host, not in a container) using the gNB config (`repo/configs/config.yaml` describes the matching CN-side slice/DNN setup — your gNB conf should reference the same SST/SD values):
...
OAI components are licensed under the OAI Public License v1.1 — see [openairinterface.org](http://www.openairinterface.org/?page_id=698).# OAI 5G Multi-UE Network Slicing Testbed (eMBB + URLLC)
